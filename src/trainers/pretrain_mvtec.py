...
os.environ['HF_HOME'] = '/scratch/latypova/huggingface/.cache'

def pretrain_mvtec(args):
    ...
    mean = stats[str(args.class_name)]['mean']
    std = stats[str(args.class_name)]['std']
    ...
    train_dataset = MVTecDataset(
        ...
        transform=T.Compose([
            ...
            T.RandomCrop((args.img_size, args.img_size), padding=4),
            T.ToTensor(),
    ...
    np.save(os.path.join(save_path, f"embeddings_epoch_{epoch + 1}.npy"), embeddings)
...
def get_class_embeddings(model, data_loader, device):
    ...
    embeddings = torch.zeros(len(data_loader.dataset), 768).to(device)  # Assuming 768 is the embedding size
    index = 0
    with torch.no_grad():
...
def get_anchor_file(anchor_path: str, args):
    if os.path.exists(anchor_path):
        return anchor_path
    else:
        config = OmegaConf.load(args.config_path)
        ckpt = torch.load(args.diffusion_ckpt, map_location=args.device) 
        ...
        text_encoder.freeeze()
        embedding_table = text_model.get_input_embeddings()
