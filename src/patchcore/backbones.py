...
def load_clip_resnet():
    model = ResNet_Model(num_classes=100)
    # weights = torch.load("/scratch/latypova/dreamood/ad/mvtec/pretrain/model_67.pth", map_location="cuda")
    # model.load_state_dict(weights)
    model.eval()
    return model

def fn_wideresnet():
    model = models.wide_resnet50_2(weights=models.Wide_ResNet50_2_Weights.IMAGENET1K_V1).to("cuda")
    model.fc = torch.nn.Identity()
    weights = torch.load("/scratch/latypova/dreamood/ad/mvtec/finetune/model_599.pt", map_location='cuda')
    model.load_state_dict(weights)
    model.eval()
    return model
...
def fn_vit():
    ...
    model.load_state_dict(torch.load("/scratch/latypova/dreamood/ad/mvtec/finetune_vit/model_5.pt", map_location="cpu"))
    return model
...
_BACKBONES = {
    ...
    "wideresnet101": "models.wide_resnet101_2(weights=models.Wide_ResNet101_2_Weights.IMAGENET1K_V1)",
    ...
    "vit_base_patch8": 'timm.create_model("vit_base_patch8_224.augreg2_in21k_ft_in1k", pretrained=True)',
    ...
    "vit_swin_large_2": 'timm.create_model("swin_large_patch4_window12_384.ms_in22k", pretrained=True)',
    "vit_large_dino": 'timm.create_model("vit_large_patch14_dinov2.lvd142m", pretrained=True)', # img size 518
    "vit_medium_relpos": 'timm.create_model("timm/vit_relpos_medium_patch16_224.sw_in1k", pretrained=True)',
    ...
    "clip_resnet": "load_clip_resnet()",
    "fn_wrn": "fn_wideresnet()",
    "fn_vit": "fn_vit()"
}
