...
class ApproximateGreedyCoresetSampler(GreedyCoresetSampler):
    ...
    def _compute_batchwise_differences(
        ...
        """Computes batchwise cosine distances using PyTorch: 1 - cos(a,b)."""
        # L2 normalize (idempotent)
        a_norm = F.normalize(matrix_a, p=2, dim=-1)
        b_norm = F.normalize(matrix_b, p=2, dim=-1)
        
        # Cosine similarity via dot product
        cos_sim = torch.mm(a_norm, b_norm.T)
        
        return 1.0 - cos_sim  # Cosine distance [0, 2]
    

class ApproximateGreedyCoresetSamplerCosine(ApproximateGreedyCoresetSampler):
    def __init__(
        self,
        percentage: float,
        device: torch.device,
