...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text, class_index, token_idx = 0):
        if text[0] != '':
            ...
            elif self.id_data in ["mvtec", "mvtec2"]:
                if isinstance(text, str):
                    text = [text]
                fine_labels = text * (class_index+1)
                print(fine_labels)
            else:
                raise NotImplementedError(f"Unknown dataset {self.id_data}")
            ...
            original_embed = deepcopy(self.transformer.text_model.embeddings.token_embedding.weight[tokens[0][token_idx + 1]])
            original_id = tokens[0][token_idx+1]
            num_of_samples = self.outlier_embedding.shape[-2]
            if False:
                ...
                if len(self.outlier_embedding.shape) == 3:
                    outlier = self.outlier_embedding[class_index][np.random.choice(10000, 1)[0]]
                else:
                    print(f"shape: {self.outlier_embedding.shape}")
                    outlier = self.outlier_embedding[np.random.choice(num_of_samples, 1)[0]]
    ...
    def encode(self, text, class_index, opt, token_idx = 0):
        self.id_data = opt.dataset
        self.outlier_embedding = torch.from_numpy(
                np.load(opt.loaded_embedding))
        return self(text, class_index, token_idx)
